...
(function search(search) {
	...
	const search_wrap = document.querySelector('#search-wrapper');
	const searchbar = document.querySelector('#searchbar');
	let searchbar_outer = document.querySelector('#searchbar-outer');
	const searchresults = document.querySelector('#searchresults');
	const searchresults_outer = document.querySelector('#searchresults-outer');
	const searchresults_header = document.querySelector('#searchresults-header');
	...
	function unfocusSearchbar() {
		...
		temporary.setAttribute('style', 'position: absolute; opacity: 0;');
		searchicon.append(temporary);
		temporary.focus();
		temporary.remove();
	}
	...
	function doSearchOrMarkFromUrl() {
		...
		if (
			...
			searchbar.value = decodeURIComponent(
				String(url.params[URL_SEARCH_PARAM]).replace(/\+/g, '%20'),
			);
		...
		if (url.params.hasOwnProperty(URL_MARK_PARAM)) {
			...
			function hide() {
				for (const marker_ of markers) {
					marker_.classList.add('fade-out');
			...
			for (const marker_ of markers) {
				marker_.addEventListener('click', hide);
			}
		}
	}
	...
	function doSearch(searchterm) {
		...
		for (let i = 0; i < resultcount; i++) {
			...
			resultElement.innerHTML = formatSearchResult(results[i], searchterms);
	...
	fetch(pathToRoot + 'searchindex.json')
		...
		.catch((error) => {
			...
			script.src = pathToRoot + 'searchindex.js';
			script.addEventListener('load', () => init(window.search));
