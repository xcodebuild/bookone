...
(function codeSnippets() {
	...
	function update_play_button(pre_block, playground_crates) {
		...
		const re = /extern\s+crate\s+(\w+)\s*;/g;
		...
		const all_available = snippet_crates.every(function (element) {
			return playground_crates.includes(element);
		});
	...
	if (window.ace) {
		...
		for (const block of code_nodes.filter(function (node) {
	...
	for (const block of Array.from(
		document.querySelectorAll('code.language-rust'),
	)) {
	...
	if (window.playground_copyable) {
		for (const block of Array.from(document.querySelectorAll('pre code'))) {
			var pre_block = block.parentNode;
	...
	for (const pre_block of Array.from(
		document.querySelectorAll('.playground'),
	)) {
		// Add play button
		var buttons = pre_block.querySelector('.buttons');
		if (!buttons) {
...
(function themes() {
	...
	const themePopup = document.querySelector('#theme-list');
...
(function sidebar() {
	...
	for (const element of Array.from(sidebarAnchorToggles)) {
	...
	sidebarToggleButton.addEventListener('click', function sidebarToggle() {
		if (html.classList.contains('sidebar-hidden')) {
			const current_width = Number.parseInt(
			...
			hideSidebar();
		} else if (getComputedStyle(sidebar).transform === 'none') {
			hideSidebar();
		} else {
			showSidebar();
		}
...
(function clipboard() {
	...
	function showTooltip(element, message) {
		element.firstChild.innerText = message;
	...
	for (const clipButton of Array.from(clipButtons)) {
	...
	(function controllPosition() {
		...
		document.addEventListener(
			...
			function () {
				...
				let nextSticky = null;
				let nextTop = null;
				const scrollDown = scrollTop > previousScrollTop;
				...
				} else if (menuPosAbsoluteY > 0) {
					nextSticky = true;
				} else if (menuPosAbsoluteY < minMenuY) {
					nextTop = previousScrollTop + minMenuY;
				}
